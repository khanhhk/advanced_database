...
def entity_cases(movies: list[dict], count: int, rng: random.Random) -> list[dict]:
    ...
    for index, target in enumerate(pool[:count]):
        ...
        decoys = [x for x in pool[count:] if x.get(label_key) and x["tmdb_id"] != target["tmdb_id"]][:4]
        candidates = decoys + ([target] if is_match else [])
        rng.shuffle(candidates)
        cases.append({"case_id": f"er-{index + 1:03d}", "left": left, "candidates": candidates,
                      "threshold": 85, "is_match": is_match, "expected_id": target["id"] if is_match else None,
                      "label_source": ("target excluded from candidate set" if not is_match else
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("experiments/labels"))
