...
def build(output_dir: Path = RESULTS / "summary") -> None:
    ...
    entity = _load("entity_resolution.json")
    reasoning = _load("reasoning.json")
    recommendation = _load("recommendation.json")["result"]
    qa = _load("qa_neo4j.json")
    semantic = _load("semantic_reasoning.json")
    validation = _load("neo4j_validation.json")
    rows = [
    ...
    with (output_dir / "quality_metrics.csv").open("w", newline="", encoding="utf-8") as stream:
        writer = csv.writer(stream); writer.writerow(["metric", "value", "evidence_class", "cases"]); writer.writerows(rows)
    ...
    with (RESULTS / "neo4j_benchmark.csv").open(encoding="utf-8", newline="") as stream:
        neo4j = list(csv.DictReader(stream))
    with (RESULTS / "relational_benchmark.csv").open(encoding="utf-8", newline="") as stream:
    ...
    with (output_dir / "benchmark_comparison.csv").open("w", newline="", encoding="utf-8") as stream:
        writer = csv.DictWriter(stream, fieldnames=comparisons[0]); writer.writeheader(); writer.writerows(comparisons)
