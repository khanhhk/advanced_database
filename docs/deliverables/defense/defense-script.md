...
| 6 | 40s | Neo4j cho operational traversal; RDF/OWL cho chuẩn trao đổi và entailment; SQLite là mốc so sánh kiểm soát. |
| 7 | 45s | Đi theo luồng nguồn → xử lý → Neo4j/RDF → API/UI/evaluation. |
| 8 | 35s | Nêu ảnh chụp dữ liệu, checksum, độ bao phủ IMDb và provenance. |
...
| 15 | 45s | Parser nhận diện 9 ý định; entity linker + query catalog quyết định Cypher. |
| 16 | 40s | IDF giảm trọng số feature phổ biến; lời giải thích lấy từ contribution thật. |
| 17 | 35s | Cùng ảnh chụp dữ liệu được xuất sang RDF/Turtle với namespace và ontology. |
