...
- Hướng sau MVP: Wikidata/giải thưởng, cải thiện tìm kiếm toàn văn và đối sánh
  gần đúng, mở rộng tập ý định, tăng dữ liệu đánh giá và thử nghiệm trên đồ thị
  lớn hơn.
...
## Việc cần kiểm tra khi hoàn thiện tệp PPTX

Phần trên là dàn ý nội dung, chưa phải tệp PPTX hoàn chỉnh. Khi dựng trang chiếu,
thực hiện năm việc sau:

1. Mỗi trang chỉ giữ một thông điệp chính; chuyển các đoạn dài thành sơ đồ, bảng
   nhỏ hoặc tối đa 3–5 gạch đầu dòng.
2. Dùng đúng số liệu của lần chạy 5.000 đầu vào: 4.999 phim hợp lệ, 76.612 nút,
   846.309 quan hệ, QA 20/20 và các kết quả đánh giá đã ghi trong báo cáo.
3. Biểu đồ phải ghi rõ dữ liệu, số lần đo và phạm vi kết luận. Không trình bày
   SQLite hay Neo4j như lựa chọn luôn nhanh hơn trong mọi trường hợp.
4. Chuẩn bị sẵn các câu hỏi và mã phim cho phần trình diễn; chạy hoàn toàn trên
   dữ liệu đã nạp, không phụ thuộc Internet.
5. Đọc thử toàn bộ một lần theo cột thời gian ở trên. Nếu vượt thời gian, rút nội
   dung ở trang 8–21, không cắt phần bài toán, kết quả, hạn chế hoặc kết luận.

Trước khi nộp, đối chiếu lần cuối số liệu và thuật ngữ giữa PPTX,
`report_latex/` và kết quả trong `experiments/results/`.

## Trang phụ lục dùng khi trả lời câu hỏi

Các trang dưới đây không nằm trong 24 trang trình bày chính. Chỉ đặt chúng sau
trang kết luận và mở khi giảng viên hỏi sâu.

### P1 — Chín ý định và Cypher tương ứng

- Bảng ánh xạ chín ý định sang mẫu Cypher: tra cứu một bước, láng giềng chung,
  tổng hợp, cạnh suy ra và đường đi ngắn nhất.
- Một ví dụ cho thấy bộ phân tích chỉ chọn ý định/tham số; cấu trúc Cypher lấy từ
  danh mục cố định và giá trị người dùng không được nối vào chuỗi truy vấn.

### P2 — Bản thể và suy diễn ngữ nghĩa

- Minh họa `domain`, `range`, `inverseOf`, quan hệ đối xứng và các ràng buộc
  functional/disjoint.
- Nêu kết quả vật chất hóa: 342.683 → 429.192 bộ ba, tăng 86.509 và không có vi
  phạm trong tập luật đã khai báo.

### P3 — Tính lũy đẳng và khả năng tái lập

- Ràng buộc/chỉ mục được tạo trước; nút được nạp trước cạnh; `MERGE` tránh nhân
  bản khi chạy lại.
- Mã kiểm tra của CSV đã xử lý và số lượng phim quyết định có cần nạp lại hay
  không.

### P4 — Phạm vi của kết quả đánh giá

- Tóm tắt cách tạo tập silver cho phân giải thực thể, QA, suy diễn và gợi ý.
- Nhấn mạnh các chỉ số chỉ có ý nghĩa trên ảnh chụp dữ liệu và quy trình gán nhãn
  đã công bố, không đại diện cho độ chính xác của mọi câu hỏi thực tế.

### P5 — Cách đo Neo4j và SQLite

- Cùng ảnh chụp dữ liệu, máy, truy vấn tương đương, một lần làm nóng và 100 lần
  đo cho mỗi truy vấn.
- SQLite nhanh hơn trong phép đo hiện tại; đóng góp của Neo4j nằm ở mô hình duyệt
  đồ thị và bằng chứng, không phải tuyên bố luôn nhanh hơn SQL.

### P6 — Câu hỏi phản biện thường gặp

- Vì sao chọn đồ thị tri thức và Neo4j?
- Vì sao dùng cả Neo4j và RDF/OWL?
- Vì sao QA chỉ hỗ trợ chín ý định?
- Điểm gợi ý và lời giải thích được tính như thế nào?
- Tên trùng hoặc nhập sai được xử lý ra sao?

Câu trả lời chi tiết nằm trong
[`../defense/defense-qa.md`](../defense/defense-qa.md).
