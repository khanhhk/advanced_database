...
VALIDATION_RESULT = Path("experiments/results/quality/neo4j_validation.json")
...
def prepare() -> dict:
    ...
    processed_sha = manifest["processed_sha256"]
    ...
    recorded_sha = prior.get("processed_sha256")
    graph_current = row["movies"] == expected_movies and recorded_sha == processed_sha
    if not graph_current:
        ...
        try:
            ...
            repository.close()
    state = {"source_sha256": source_sha, "processed_sha256": processed_sha,
