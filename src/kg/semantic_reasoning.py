...
The operational graph uses Neo4j.  This module makes the standards comparison
executable: it materializes the entailments used by this ontology and emits a
machine-readable validation report without requiring an external network service.
"""
...
def materialize(data: Graph, ontology: Graph) -> dict:
    ...
    for subject, predicate, obj in list(data):
        ...
        if predicate in ranges and not str(ranges[predicate]).startswith("http://www.w3.org/2001/XMLSchema"):
    ...
    for triple in inferred:
        data.add(triple)
    return {"triples_before": before, "triples_after": len(data),
...
def validate(data: Graph, ontology: Graph) -> dict:
    ...
    for prop in ontology.subjects(RDF.type, OWL.FunctionalProperty):
        ...
        for subject, objects in values.items():
            if len(objects) > 1:
                violations.append({"type": "functional_property", "subject": str(subject),
                                   "property": str(prop), "value_count": len(objects)})

    disjoint = set(ontology.subject_objects(OWL.disjointWith))
    for subject in set(data.subjects(RDF.type, None)):
        ...
        for left, right in disjoint:
            if left in types and right in types:
                violations.append({"type": "disjoint_classes", "subject": str(subject),
                                   "classes": [str(left), str(right)]})

    movie = next(ontology.subjects(RDF.type, OWL.Class), None)
    namespace = str(ontology.identifier) if ontology.identifier else ""
    # Resolve by local names so validation remains stable if the base URI changes.
    movie_types = [x for x in ontology.subjects(RDF.type, OWL.Class) if str(x).endswith("/Movie")]
    title_props = [x for x in ontology.subjects(RDF.type, OWL.DatatypeProperty) if str(x).endswith("/title")]
    ...
    return {"conforms": not violations, "violation_count": len(violations),
            "violations": violations}


def run(ontology_path: Path, input_path: Path, output_path: Path, report_path: Path) -> dict:
    ontology, data = Graph(), Graph()
    ontology.parse(ontology_path)
    data.parse(input_path)
    metrics = materialize(data, ontology)
    report = {"profile": "rdfs-owl-rl-subset", **metrics,
